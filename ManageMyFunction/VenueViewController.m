...
- (void)viewDidLoad {
    ...
    //_event.text = [[NSUserDefaults standardUserDefaults] objectForKey:@"eventName"];
...
-(IBAction)areaselectClicked:(id)sender {
    ...
    // NSString *savedvalue2=[[NSUserDefaults standardUserDefaults]stringForKey:@"city_id"];
    ...
    NSString *myst2=[NSString stringWithFormat:@"city_id=%@&event_name=%@&area_id=%@",self.city_id.text,self.event.text,self.area_id.text];
    
    NSLog(@"POST CONTAINT AREA....%@",myst);
...
- (void)niDropDownDelegateMethod: (NIDropDown *) sender {
    ...
    if (areaBtnSelect.titleLabel.text!=NULL) {
        ...
        _area_idstr =_area_idarr[anIndex];
...
-(void)dataParsing
{
    ...
    NSString *myst=[NSString stringWithFormat:@"start_date=%@&end_date=%@&start_time=%@&end_time=%@&city=%@&event=%@",self.start_date.text,self.end_date.text,self.start_time.text,self.end_time.text,self.city.text,self.event.text];
    ...
    NSURL * url = [NSURL URLWithString:@"http://192.168.1.17/mmf_apis/get_services1.php"];//for booked and available button api
...
-(void)getFilterVenue
{
    ...
    NSString *myst=[NSString stringWithFormat:@"city_id=%@&event_name=%@&area_id=%@&venue_type_id=%@&allow_other_caterer=%@&capacity1=%@&capacity2=%@",self.city_id.text,self.event.text,self.area_id.text,self.venue_type_id.text,self.allow_other_caterer.text,self.capacity1.text,self.capacity2.text];
...
{
    VenueTableViewCell *cell=[_tableview dequeueReusableCellWithIdentifier:@"cell" forIndexPath:indexPath];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    VenueTableViewCell *cell=[_tableview cellForRowAtIndexPath:indexPath];
    ...
    //    //  NSString *tempstr=@"2";
    //
    //    //  NSString *tempstr= [[_prog_id_arr objectAtIndex:indexPath.row]description];
    //    //NSLog(@"%@",[_prog_id_arr objectAtIndex:indexPath.row]);
    //
    //    if([tempstr isEqualToString:@"1"])
    //    {
    //        [self performSegueWithIdentifier:@"showSessionNew" sender:self];
    //    }
