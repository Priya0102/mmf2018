...
#import "CancelledUser.h"
#import "CancelledOtherUser.h"
#import "CancelledUserTableViewCell.h"
#import "CancelledOtherUserTableViewCell.h"
@interface CancelledUserViewController ()

@end

@implementation CancelledUserViewController
            ...
            for (NSDictionary *temp in arr) {
                ...
                NSLog(@"book_id=%@ \n service_provider_id=%@ \n user_event_id=%@ \n user_id=%@ \n event_type_id=%@ \n event_date_from=%@ \n event_date_to=%@ \n event_time_from=%@ \n event_time_to=%@ \n no_of_guests=%@ \n requested_date=%@ \n is_paid=%@\n is_confirmed=%@ \n  service_provider_user_id=%@ \n first_name=%@ \n last_name=%@ \n contact=%@ \n Service Name==%@ \n Established yr==%@ \n event_name=%@",str1,str2,str3,str4,str5,str6,str7,str8,str9,str10,str11,str13,str14,str15,str16,str17,str18,str20,str21,str22);
                {
                    [[NSOperationQueue mainQueue]addOperationWithBlock:^{
                        ...
                        //                        if ([_is_ConfirmedStr isEqualToString:@"0"]) {
                        //                            NSLog(@"Cancelled...");
                        ...
                        //                        }
                        //                        if ([_is_ConfirmedStr isEqualToString:@"1"]) {
                        //                            NSLog(@"Pending...");
...
-(void)getOtherFacility{
    ...
    NSURLSessionDataTask *task=[sess dataTaskWithRequest:req completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        if (data==nil) {
            ...
            for(NSDictionary *temp in arr){
                CancelledUser *f=[[CancelledUser alloc]init];
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (tableView==self.tableview) {
        CancelledUserTableViewCell *cell=[_tableview dequeueReusableCellWithIdentifier:@"cell" forIndexPath:indexPath];
        
        CancelledUser *f=self.otherfaciltyarr[indexPath.row];
        ...
        CancelledOtherUserTableViewCell *cell=[_tableview2 dequeueReusableCellWithIdentifier:@"cell" forIndexPath:indexPath];
        
        CancelledOtherUser *o=self.otherdetailarr[indexPath.row];
...
-(void)getOtherDetailsOfVenue{
    ...
    NSURLSessionDataTask *task=[sess dataTaskWithRequest:req completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        if (data==nil) {
            ...
            for(NSDictionary *temp in arr){
                CancelledOtherUser *o=[[CancelledOtherUser alloc]init];
            ...
            for(NSDictionary *temp in arr){
                ...
                NSLog(@"Comment book id==%@",str1);
                {
                    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
                        ...
                        _bookid.text=_book_id_str;
                        if(_book_id_str==(NSString *) [NSNull null])
                        {
                            _book_id_str=@"-";
                        }
                        else{
